...
const BotonFilas = ({ titulo, valor, tipo = true, top = 0 }: Props) => {
  ...
  const label = titulosGenerala[titulo];
  const diceIndex = label.search(/[⚀-⚅]/);
  const textPart = diceIndex >= 0 ? label.slice(0, diceIndex).trim() : label;
  const dicePart = diceIndex >= 0 ? label.slice(diceIndex) : '';
    ...
    >
      <View
        style={{
          ...
          ...(dicePart ? { height: undefined, minHeight: 45 } : {}),
        }}
      >
        <Text
          style={{
      ...
      {[0, 1, 2, 3, 4, 5].map((i) => (
        <View
          ...
          style={{
            ...generala.columna2,
            borderColor: theme.border,
            ...(dicePart ? { height: undefined, alignSelf: "stretch" } : {}),
          }}
        >
          <Pressable
            onPress={() => {
